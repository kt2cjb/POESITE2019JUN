<?php
class XooUserAdminLite extends XooUserUltraCommonLite
{
	...
	function __construct() {
		...
		$this->plugin_data = get_plugin_data( xoousers_lite_path . 'xoousers.php', false, false);
	...
	function add_styles()
	{
		...
		wp_register_style('userultra_admin', xoousers_lite_url.'admin/css/userlutra.admin.css');
		wp_enqueue_style('userultra_admin');			
			
		wp_register_script('userultra_chosen', xoousers_lite_url . 'admin/scripts/admin-chosen.js');
		wp_enqueue_script('userultra_chosen');
		
		
		//color picker		
		 wp_enqueue_style( 'wp-color-picker' );		 
		 wp_register_script( 'userultra_color_picker', xoousers_lite_url.'admin/scripts/color-picker-js.js', array( 
			'wp-color-picker'
		) );
		wp_enqueue_script( 'userultra_color_picker' );
		
		wp_register_script( 'userultra_admin', xoousers_lite_url.'admin/scripts/admin.js', array( 
	...
	function add_menu() 
	{
		...
		add_menu_page( __('Users Ultra','xoousers'), $menu_label, 'manage_options', $this->slug, array(&$this, 'admin_page'), xoousers_lite_url .'admin/images/small_logo_16x16.png', '159.140');
	...
	function include_tab_content() 
	{
		...
		if( strstr($screen->id, $this->slug ) ) 
		{
			...
			{
				require_once (xoousers_lite_path.'admin/tabs/'.$tab.'.php');				
			}else{
