...
[usersultra_directory list_per_page=8 optional_fields_to_display='friend,social,country,description' pic_boder_type='rounded']",
	...
	{
		//load_plugin_textdomain( 'xoousers', false, xoousers_lite_path.'/languages/');
    }
	...
	{	
		...
		{
			if (file_exists(xoousers_lite_path."xooclasses/$class.php")) 
			{
				require_once(xoousers_lite_path."xooclasses/$class.php");
	...
	public function add_front_end_styles()
	{
        ...
        if (!wp_script_is('uultra_tipsy')) {
			wp_register_script('uultra_tipsy', xoousers_lite_url.'js/jquery.tipsy.js',array('jquery'));
		...
		/* Font Awesome */
		wp_register_style( 'xoouserultra_font_awesome', xoousers_lite_url.'css/css/font-awesome.min.css');
		wp_enqueue_style('xoouserultra_font_awesome');			
		
		/* Main css file */
		//wp_register_style( 'xoouserultra_css', xoousers_lite_url.'templates/'.xoousers_lite_template.'/css/xoouserultra.css');
		//wp_enqueue_style('xoouserultra_css');		
				
		/* Custom style */		
		wp_register_style( 'xoouserultra_style', xoousers_lite_url.'templates/'.xoousers_lite_template.'/css/default.css');
		wp_enqueue_style('xoouserultra_style');						
		
			
		/*Expandible*/		
		wp_register_script( 'xoouserultra_expandible_js', xoousers_lite_url.'js/expandible.js',array('jquery'));
		...
		{
			//lightbox
			//wp_register_style( 'xoouserultra_lightbox_css', xoousers_lite_url.'js/lightbox/css/lightbox.css');
			//wp_enqueue_style('xoouserultra_lightbox_css');
			
			//wp_register_script( 'xoouserultra_lightboxjs', xoousers_lite_url.'js/lightbox/js/lightbox-2.6.min.js',array('jquery'));
			//wp_enqueue_script('xoouserultra_lightboxjs');
		}
		
		/*Validation Engibne JS*/		
			
		wp_register_script( 'form-validate-lang', xoousers_lite_url.'js/languages/jquery.validationEngine-en.js',array('jquery'));			
		wp_enqueue_script('form-validate-lang');
					
		wp_register_script('form-validate', xoousers_lite_url.'js/jquery.validationEngine.js',array('jquery'));
		wp_enqueue_script('form-validate');
		
		
		/*Users JS*/		
		wp_register_script( 'uultra-front_js', xoousers_lite_url.'js/uultra-front.js',array('jquery'));
...
$date_picker_array = array(
	...
	public function password_reset( $args=array() ) {
		...
		$display .= '<div class="xoouserultra-head">';
		    $display .='<div class="xoouserultra-left">';
		        $display .='<div class="xoouserultra-field-name xoouserultra-field-name-wide login-heading" id="login-heading-'.$this->login_code_count.'">'.__('Password Reset','xoousers').'</div>';
		    $display .='</div>';
			...
			if ( $type == 'usermeta') {
				...
				$display .= '<div class="xoouserultra-field-value">';
					...
					switch($field) {
						...
						case 'textarea':
							$display .= '<textarea placeholder="'.$name.'" class="'.$required_class.' xoouserultra-input" name="'.$meta.'" id="reg_'.$meta.'" title="'.$name.'" data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'">'.$this->get_post_value($meta).'</textarea>';
							break;
						
						case 'text':
							$display .= '<input placeholder="'.$name.'" type="text" class="'.$required_class.' xoouserultra-input" name="'.$meta.'" id="reg_'.$meta.'" value="'.$this->get_post_value($meta).'" title="'.$name.'" data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'"/>';
	...
	public function show_usersultra_my_account($atts )
	{
		...
		require_once(xoousers_lite_path.'/templates/'.xoousers_lite_template."/dashboard.php");
	...
	{
		$display = "";
		$display .= require_once(xoousers_lite_path.'/templates/'.xoousers_lite_template."/".$template.".php");	
	...
	function get_facebook_sdk($api)
	{
		...
		require_once(xoousers_lite_path."libs/fbsdkjs/fbscript.php");
		...
		if($this->get_option('registration_rules')!=4) // Social media is not able when using paid registrations
		{
			...
			if($this->get_option('social_media_fb_active')==1)
			{
				...
				$display .='<div class="txt-center FacebookSignIn">
						...
						<a href="#" id="uupro-facebook-login-bt" class="btnuultra-facebook" >
							<span class="uultra-icon-facebook"> <img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/facebook.png" ></span>'.$action_text.' Facebook </a>
			...
			if($this->get_option('social_media_yahoo')==1)
			{
				...
				$display .='<div class="txt-center YahooSignIn">	               	
							...
							<span class="uultra-icon-yahoo"><img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/yahoo.png" ></span>'.$action_text.' Yahoo </a>
			...
			if($this->get_option('social_media_google')==1)
			{
				...
				$display .='<div class="txt-center GoogleSignIn">	               	
						<a href="'.$auth_url_google.'" class="btnuultra-google" >
							<span class="uultra-icon-google"><img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/googleplus.png" ></span>'.$action_text.' Google </a>
				...
				$atleast_one = true;			
					...
					$display .='<div class="txt-center InstagramSignIn">	               	
						<a href="'.$auth_url_google.'" class="btnuultra-instagram" >
							<span class="uultra-icon-instagram"><img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/instagram-icon.png" ></span>'.$action_text.' Instagram </a>
				...
				if($display_style=='minified')
				{
					...
					$display .='               	
						<a href="'.$auth_url_google.'" class="btnuultramini-yammer '.$rounded_class.'" >
							<span class="uultra-icon-yammer"><img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/yammer.png" ></span> </a>
					...
					$display .='<div class="txt-center YammerSignIn">	               	
						<a href="'.$auth_url_google.'" class="btnuultra-yammer" >
							<span class="uultra-icon-yammer"><img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/yammer.png" ></span>'.$action_text.' Yammer </a>
			...
			if($this->get_option('twitter_connect')==1)
			{
				...
				$display .='<div class="txt-center TwitterSignIn">	               	
						<a href="'.$auth_url_google.'" class="btnuultra-twitter" >
							<span class="uultra-icon-twitter"><img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/twitter.png" ></span>'.$action_text.' Twitter </a>
			...
			if($this->get_option('social_media_linked_active')==1)
			{
				...
				$display .='<div class="txt-center LinkedSignIn">	               	
							<a href="'.$requestlink.'" class="btnuultra-linkedin" >
								<span class="uultra-icon-linkedin"><img src="'.xoousers_lite_url.'templates/'.xoousers_lite_template.'/img/socialicons/linkedin.png" ></span>'.$action_text.' LinkedIn </a>
	...
	public function load_google()
	{
		if ( $this->get_option('social_media_google') == 1 && $this->get_option('google_client_id') && $this->get_option('google_client_secret') && $this->get_option('google_redirect_uri') ) 
		{
			...
			require_once(xoousers_lite_path."libs/google/src/Google/Client.php");
			require_once(xoousers_lite_path."libs/google/src/Google/Service/Plus.php");
			require_once(xoousers_lite_path."libs/google/src/Google/Service/Oauth2.php");		
			session_start();
	...
	******************************************/
	...
	public function social_login_links_openid()
	{
		...
		{						
			...
			require_once(xoousers_lite_path."libs/openid/openid.php");				
	...
	public function get_linkein_auth_link ()
	{
		...
		//LinkedIn lib
		 require_once(xoousers_lite_path."libs/linkedin/oauth/linkedinoauth.php");		 
