...
class UsersUltraPhotoCateLite{
	...
	function __construct() {
		...
		$this->plugin_data = get_plugin_data( uultra_dg_path_lite . 'index.php', false, false);
	...
	function add_styles(){
		...
		wp_register_script( 'uultra_cate_js', uultra_dg_url_lite . 'admin/scripts/admin.js', array( 
			'jquery'
		) );
		wp_enqueue_script( 'uultra_cate_js' );
	
		wp_register_style('uultra_cate_css', uultra_dg_url_lite . 'admin/css/admin.css');
	...
	function get_tab_content() {
		...
		if( strstr($screen->id, $this->subslug ) ) {
			...
			require_once uultra_dg_path_lite.'admin/panels/'.$tab.'.php';
		}
	}
...
$uultra_photo_category_lite = new UsersUltraPhotoCateLite();