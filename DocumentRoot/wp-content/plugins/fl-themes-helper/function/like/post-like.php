...
function sl_enqueue_scripts() {
    wp_enqueue_script('simple-likes-public-js',plugin_dir_url( __FILE__ ).'/js/likes-public.js', false, null , true);
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like' => esc_html__( 'Like', 'fl-themes-helper' ),
		'unlike' => esc_html__( 'Unlike', 'fl-themes-helper' )
	) );
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'fl-themes-helper' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'fl-themes-helper' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'fl-themes-helper' );
		$icon = $icon_empty;
	}
	$output = '<span class="sl-wrapper"><a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&post_id=' . $post_id . '&nonce=' . $nonce . '&is_comment=' . $is_comment . '&disabled=true' ) . '" class="sl-button' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $count . $icon . '</a></span>';
	return $output;
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'fl-themes-helper' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'fl-themes-helper' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
