...
if ( !class_exists( 'BuddyForms' ) ) {
    ...
    class BuddyForms
    {
        ...
        function admin_js( $hook_suffix )
        {
            ...
            if ( isset( $post ) && ($post->post_type == 'buddyforms' || $post->post_type == 'post') && isset( $_GET['action'] ) && $_GET['action'] == 'edit' || isset( $post ) && $post->post_type == 'buddyforms' && $hook_suffix == 'post-new.php' || $hook_suffix == 'buddyforms_page_buddyforms_submissions' || $hook_suffix == 'buddyforms_page_buddyforms_settings' ) {
                wp_register_script(
                    'buddyforms-admin-js',
        ...
        function front_js_loader()
        {
            ...
            if ( !empty($post->post_content) ) {
                $form_slug = buddyforms_get_form_slug_from_content( $post->post_content, array(
                    'bf-list-submissions',
                    'buddyforms_form',
                    'buddyforms_list_all',
                    'buddyforms_the_loop',
        ...
        public static function front_js_css( $content = '' )
        {
            ...
            wp_enqueue_style( 'buddyforms-select2-css', plugins_url( 'assets/resources/select2/dist/css/select2.min.css', __FILE__ ) );
            wp_enqueue_script(
                'buddyforms-datetimepicker',
                plugins_url( 'assets/resources/datetimepicker/jquery.datetimepicker.full.min.js', __FILE__ ),
                array( 'jquery' ),
                BUDDYFORMS_VERSION
            );
